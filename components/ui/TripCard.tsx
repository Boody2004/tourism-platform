...
export default function TripCard({ trip, variant = "default" }: TripCardProps) {
  if (variant === "horizontal") {
    return (
      <Link
        href={`/trip/${trip.slug}`}
        className="group flex gap-4 bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5"
      >
        ...
        <div className="flex flex-col justify-center py-4 pr-4 gap-1">
          ...
          <h3 className="font-display font-semibold text-dark-800 text-base leading-tight group-hover:text-brand-600 transition-colors">
            {trip.title}
          </h3>
          ...
          <p className="text-brand-600 font-bold text-sm mt-1">
            From USD {trip.price.toLocaleString()}
          </p>
        </div>
      </Link>
    );
    ...
    >
      ...
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3">
          <p className="text-white/80 text-xs">From per adult</p>
          <p className="text-white font-bold text-lg">
            USD {(trip.adultPrice ?? trip.price).toLocaleString()}
          </p>
        </div>
      </div>
